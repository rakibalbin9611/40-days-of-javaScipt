...
function selectAnswer(index, shouldScore) {
  clearInterval(timer);
  const q = questions[currentQuestion];
  const buttons = document.querySelectorAll(".option-btn"); // retrived all buttons

  buttons.forEach((button) => (button.disabled = true)); // after selecting, rest of the button will be disabled.
  if (index === q.correct) {
    shouldScore && score++;
  ...
  nextBtn.style.display = "inline-block"; // after selecting next btn will be visible.
}
